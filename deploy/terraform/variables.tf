...
variable "dns_zone_name" {
  description = "Cloudflare zone (root domain) the app record is created in"
  type        = string
  default     = "nobuddy.org"
}

variable "subdomain" {
  description = "Subdomain the app is served on - the whole app (UI + API) lives on one origin here, unlike RideMergeBuddy's split frontend/backend"
  type        = string
  default     = "gamegallerybuddy"
}
